...
async function notifyPick(redis, p, { sentKey, titlePrefix = "", tagPrefix = "sv-pick" } = {}) {
  ...
  // player name leads the title so iOS never truncates it (the "from Draft Day"
  // line under it is Safari's mandatory web-push attribution — not ours to change)
  return sendToAll(redis, {
    title: `${titlePrefix}${p.player}`,
    body: `SV CLIENT DRAFTED — Pick #${p.overall} (${p.team || p.teamName || "?"}) · Rd ${p.round}${slot}`,
